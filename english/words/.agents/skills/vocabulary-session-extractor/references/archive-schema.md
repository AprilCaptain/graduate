...
每日文件在首个“第 N 次训练”之前保留一个文件级“今日复习”章节。将含有至少一个正式 `record_id` 和训练章节的日期文件视为实际学习日；按日期排序后，从当天向前取第 1、2、4、7、15 个实际学习日，不按自然日补齐断学日期。使用：
...
任务按间隔由近到远排列，链接文字使用待复习文件的日期，链接只从当天文件指向历史文件。没有历史复习目标时，在受控区块中写 `暂无需复习内容。`，不要创建虚假任务。任务初次生成使用 `[ ]`；后续更新保留同一目标已有的 `[x]`，但不把勾选状态写入单词详情或元数据。

创建或更新当天文件后运行：

```bash
python3 .agents/skills/vocabulary-session-extractor/scripts/update_daily_reviews.py \
  --date YYYY-MM-DD
```

回填全部历史文件或补录早于现有最新学习日的记录时使用 `--all`，校验而不写入时追加 `--check`。脚本只更新受控注释之间的内容；保留章节内其他人工内容和整份文件的其他章节。重复运行不得产生差异。
...
表格中的每个单词只出现一次，核心含义保持简洁。每项核心含义必须标注词性；有多个词性时，以对应单词详情中的“核心词义”表为准，保持相同顺序并使用 `<br>` 在同一单元格内分行，不为不同词性重复创建单词行。复习单词沿用项目现有格式，除非用户另行指定。

“代表性训练句”中的每条句子必须同时包含完整英文原句和自然准确的中文译文。统一使用一个编号项内的两行格式，不使用只有英文的单行条目：

```markdown
1. **原句**：Students acquire knowledge through reading and practice.<br>
   **译文**：学生通过阅读和练习获取知识。
```
...
不要添加 `status`、`mastery_level`、正确率或复习状态。保留其他原有 YAML 字段和人工内容。文件至少维护核心词义、常用搭配、句子训练和按日期列出的学习记录。核心词义表只保留“词性”和“核心含义”两列，不添加“本次语境”列；每种词性独占一行，不使用 `n. / v.` 等聚合写法。一个单词有多个词性时，按词性拆成多行，并将对应含义写入各自行。单词详情不创建独立的“易错点”章节，有复习价值的错误提醒汇总到每日训练记录。学习记录关联 `record_id`，以日期、来源和规范化内容去重。
